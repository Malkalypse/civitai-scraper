...
class ImageCacheManager {
	 ...
	 * @return array Associative array of derived cache naming/path values
	 */
	public function buildCachePathsForImage( $imageUrl, $imageId ): array {
		$matches = [];
	 ...
	 * @param mixed $imageId Image ID value
	 * @param mixed $payload Associative array of metadata fields to merge
	 */
	...
	/** Load generation metadata indexed by model, optionally scoped to one model.
	 * @param mixed $targetModelId Optional model ID to filter results for
	 * @return array Two-element array: [byModel map, scoped entries for targetModelId]
	 */
	public function loadGenerationMetadataByModel( $targetModelId = null ): array {
		/** @var array<string, array<int, bool>> $byModel */
		$byModel     = [];
		$modelScoped = [];
