...
class FileUtils {
	 ...
	 */
	public static function normalizeFilenames( string $oldFilename, string $newFilename, ?string $originalFilename = null ) {
		$extension    = self::getExtension( $originalFilename ) ?? '.safetensors';
		$oldFilename  = self::stripExtension( $oldFilename );
		$newFilename  = self::stripExtension( $newFilename );
	 ...
	 */
	public static function findFileRecursively( string $rootPath, string $targetFilename ): ?string {
		return self::findEntryRecursively( $rootPath, $targetFilename, false );
	}
	 ...
	 */
	public static function findDirectoryRecursively( string $rootPath, string $targetDirectoryName ): ?string {
		return self::findEntryRecursively( $rootPath, $targetDirectoryName, true );
	}
	 ...
	 */
	public static function findEntryRecursively( string $rootPath, string $targetName, bool $findDirectory ): ?string {
    ...
    // Use RecursiveDirectoryIterator to traverse the directory tree
		$iterator = new RecursiveIteratorIterator(
			new RecursiveDirectoryIterator( $rootPath, RecursiveDirectoryIterator::SKIP_DOTS ),
			RecursiveIteratorIterator::SELF_FIRST
		);

    // Check each entry for name match of correct type
		foreach( $iterator as $entry ) {

      // Determine if entry matches desired type (file or directory)
			$matchesType = $findDirectory
        ? $entry->isDir()
        : $entry->isFile();

      // If name matches and type is correct, return full path
	 ...
	 */
	public static function buildRenameTargetsResult(
		string  $rootPath,
		string  $oldPath,
		string  $newPath,
	 ...
	 */
	public static function findRenameTargets(
		array   $searchRoots,
		bool    $isFolderMode,
		string  $oldFilename,
		string  $newFilename,
		...
		if( $scopedSubdir ) {
			foreach( $searchRoots as $rootPath ) {
				$scopedResult = self::findRenameTargetsInRoot(
					$rootPath,
					$isFolderMode,
					$oldFilename,
					$newFilename,
		...
		foreach( $searchRoots as $rootPath ) {
			$fallbackResult = self::findRenameTargetsInRoot(
				$rootPath,
				$isFolderMode,
				$oldFilename,
				$newFilename,
	 ...
	 * @param bool        $restrictToScope  If true, only search within the scoped subdirectory; if false, search entire root
	 * @return array|null Structured result with old/new paths and context, or null if no targets
	 */
	public static function findRenameTargetsInRoot(
		string  $rootPath,
		bool    $isFolderMode,
		string  $oldFilename,
		string  $newFilename,
		...
		if( $isFolderMode ) {
			...
			if( is_dir( $directMatch ) ) {
				return self::buildRenameTargetsResult(
					$rootPath,
					$directMatch,
			...
			$foundDir = self::findDirectoryRecursively( $searchFolder, $oldFilename );
			if( $foundDir ) {
		...
		$foundFile = self::findFileRecursively( $searchFolder, $oldFilenameWithExt );
		if( $foundFile ) {
